...
if (table) {
  ...
  function setTextareaHeight() {
    if (table) {
      ...
      const pagination = document.querySelector(".pagination-wrapper") as HTMLDivElement;
      const paginationHeight = pagination?.offsetHeight ?? 0;

      const footerSpacer = document.querySelector(".footer-spacer") as HTMLDivElement;
      const footerSpacerHeight = footerSpacer?.offsetHeight ?? 0;

      const footerHeight = document.querySelector("footer")?.offsetHeight ?? 0;

      const occupiedHeight = elDistanceFromTop + paginationHeight + footerSpacerHeight + footerHeight;
